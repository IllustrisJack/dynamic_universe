...
Once a vassalage forms, a garrison fleet (1 destroyer + 2 frigates + 4 fighters, race-matched to the suzerain) dispatches from the suzerain's actual shipyard to the vassal's HQ sector plus up to two more vassal-owned sectors. Tribute is a happiness lever and a player-payout: each tick the vassal's computed Worth × tribute % is the nominal tax, and the vassal feels the bite via the `tribute_resentment` happiness event and a small continuous drain. When the player is the suzerain, half the nominal tribute lands in the wallet, capped at 5M Cr/tick. AI-suzerain tribute doesn't move credits — AI factions have unlimited Cr in X4 so a virtual ledger would be theatre; tribute exists for happiness pressure only.

Happiness ticks every ~5 min. Drains from high tribute, sectors lost, and a "crisis" layer when the suzerain's military falls below 1.2× the combined vassal military. A 25-min event lottery layers in named events (cultural exchange, trade boom, political scandal, tribute resentment, …). AI suzerains auto-gift to stabilize unhappy vassals — costs `vassal_worth / 2000` and is gated by the suzerain's own Worth (1% per fire), so a smashed suzerain genuinely can't keep buying off rebellion. Below 30 happiness, rebellion rolls fire — successful rebels defect with the garrison ships, get a 60 min cooldown, and may attract supporter fleets from hostile-to-former-suzerain factions.
